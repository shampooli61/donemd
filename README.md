...
- **飞书桥（Feishu bridge）**：本地 `.md` 与飞书文档双向同步——标题 / 列表 / 表格 / 高亮块 / Mermaid 走标准 Markdown，飞书独有的块（电子表格 / 画板 / 多维表格等）以占位块保留。支持的内容可双向同步；嵌套原生块、公式和原文块会在推送前明确提示限制，避免静默丢失。<sub>（部分格式与排版细节两侧可能略有差异，同步能力以文档中的提示为准，本地视频不上传。）</sub>
...
`⌘⌥S` 把当前文档同步进飞书，块结构（标题 / 表格 / callout / 图片）保真；`⌘⌥O` 从飞书拉回本地。支持类型的 Markdown 双向同步，飞书独有块以占位保留。
